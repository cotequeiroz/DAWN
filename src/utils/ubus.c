...
// hostapd inserts this list backwards, so we must start with the regular APs, then
// add the preferred ones, which are already ordered backwards.
int ap_get_nr(struct blob_buf *b_local, struct dawn_mac own_bssid_addr, const char *ssid) {
    ...
    if (!own_ap)
        return -1;
    strncpy(channel_str, own_ap->neighbor_report + NR_CHANNEL, 2);
    channel_str[2] = '\0';
    neighbor_list = strtol(channel_str, NULL, 16) > 14 ?
